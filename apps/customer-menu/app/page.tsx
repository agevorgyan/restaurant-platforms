...
export default function Home() {
  ...
  const handleAddToCart = (
    ...
    const customsPart = Object.entries(customizations)
      .flatMap(([, choices]) => choices.map((c) => c.id))
      .sort()
      .join('-');
  ...
  const filteredItems = MENU_ITEMS.filter((item) => {
    ...
    const categoryMatches =
      item.dietary.includes(activeCategory as DietaryTag) ||
        ...
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-6 text-xs text-neutral-500 font-medium">
          ...
          <div className="text-center md:text-right space-y-1.5">
            ...
            <p className="text-[10px] text-neutral-600">
              Prices exclude {Math.round(RESTAURANT_CONFIG.fees.taxRate * 100)}% state tax and
              discretionary {Math.round(RESTAURANT_CONFIG.fees.serviceChargeRate * 100)}% service
              charge.
            </p>
          </div>
        </div>
